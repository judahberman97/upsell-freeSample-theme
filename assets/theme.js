...
(function (FlickityFade, bodyScrollLock, MicroModal, Rellax, Flickity, themeCurrency, Sqrl, axios, themeAddresses, ellipsed, FlickitySync, AOS) {
  ...
  class Drawer {
    ...
    showDrawer() {
      ...
      setTimeout(() => {
        if(typeof fire_cart_data !== 'undefined'){ fire_cart_data(1, 'all'); }
      }, 800);
    }
  ...
  const selectors$z = {
    ...
    addQuantityDetached: 'data-add-to-cart-quantity',
    ...
    initDetached() {
      this.button.addEventListener(
        ...
        function (evt) {
          ...
          let formString = `form_type=product&id=${variant}`;

          const quantity = this.button.getAttribute(selectors$z.addQuantityDetached);
          formString += `&quantity=${quantity}`;
  ...
  const selectors$fp = {
    freeProductHolder: 'data-free-product-holder',
    addButtonWrapper: '[data-add-action-wrapper]',
    cartLineItems: '[data-line-items]',
    focusable: 'button, [href], select, textarea, [tabindex]:not([tabindex="-1"])',
  };

  class FreeProduct extends HTMLElement {
    constructor() {
      super();

      this.freeProductHolder = this.querySelector(`[${selectors$fp.freeProductHolder}]`);
      
      if (this.freeProductHolder) {
        this.isCartItem = Boolean(this.freeProductHolder.closest(selectors$fp.cartLineItems));
    ...
    init() {
      const totalPrice = document.querySelector(`[data-line-items] .cart__items`).getAttribute('total-final-price');

      if (document.querySelector('.free-sample-collection--wrapper .free-sample-collection__title')) {
        if (parseInt(totalPrice) >= 7500) {
          document.querySelector('.free-sample-collection--wrapper .free-sample-collection__title').innerHTML = "Choose 2 free samples";
        } else {
          document.querySelector('.free-sample-collection--wrapper .free-sample-collection__title').innerHTML = "Choose your free sample";
        }
      }

      if (this.isCartItem) {
        new ProductAddButton(this.freeProductHolder, this.isCartItem);
      }

      const variantId = this.getAttribute('data-variant-id');

      if (document.querySelector(`[data-line-items] [data-cart-item-id="${variantId}"]`)) {
        this.classList.add('added');
        document.querySelector(`[data-line-items] [data-cart-item-id="${variantId}"] .cart__items__price`).classList.add('hide');
        document.querySelector(`[data-line-items] [data-cart-item-id="${variantId}"] .cart__items__quantity`).classList.add('hide');
      }
      ...
      this.addEventListener('click', function(e){
        ...
        if (parseInt(totalPrice) >= 7500 && document.querySelectorAll(`[data-line-items] free-product.added`).length < 2) {
          this.querySelector('[data-add-to-cart]').click();
        } else if (parseInt(totalPrice) <= 7500 && document.querySelectorAll(`[data-line-items] free-product.added`).length < 1) {
          this.querySelector('[data-add-to-cart]').click();
        }
      });
    }
  ...
  const selectors$pqac = {
    pqacHolder: 'data-quick-add-to-cart-holder',
    addButtonWrapper: '[data-add-action-wrapper]',
    modalContent: '[data-product-upsell-ajax]',
    upsellModal: '[data-upsell-modal]',
    upsellModalTemplate: '[data-upsell-modal-template]',
  ...
  class ProductQuickAddToCartForm extends HTMLElement {
    constructor() {
      super();

      this.pqacHolder = this.querySelector(`[${selectors$pqac.pqacHolder}]`);
      //this.modalCloseEvent = () => this.modalClose();

      if (this.pqacHolder) {
        // this.isCartItem = Boolean(this.pqacHolder.closest(selectors$pqac.cartLineItems));
        // this.modalTemplate = this.pqacHolder.querySelector(selectors$pqac.upsellModalTemplate);
        // this.modal = document.querySelector(selectors$pqac.upsellModal);
        // this.modalID = this.pqacHolder.getAttribute(selectors$pqac.pqacHolder);
        // this.modalContent = null;
        // this.triggerButton = document.querySelector(`[data-popup-${this.modalID}]`);
        // this.handle = this.triggerButton ? this.triggerButton.getAttribute(`data-popup-${this.modalID}`) : null;
        ...
        //   const htmlObject = document.createElement('div');
        //   htmlObject.innerHTML = modalTemplateInner;
        //   this.modalHtml = htmlObject.querySelector(selectors$pqac.upsellModal);
        //   document.body.appendChild(this.modalHtml);
        //   this.modal = document.querySelector(selectors$pqac.upsellModal);
        // }
    ...
    init() {
      const addButtonWrapper = this.querySelector(selectors$pqac.addButtonWrapper);
      new ProductAddButton(addButtonWrapper, this.isCartItem);
      // if (this.modalTemplate && this.triggerButton) {
      //   this.triggerButton.addEventListener('click', (e) => {
      //     e.preventDefault();
    ...
    initButtons() {
      this.increase.addEventListener(
        ...
        function (e) {
          ...
          if (this.wrapper.closest('.upsell__button__content').querySelector('button[data-add-to-cart')) {
            this.wrapper.closest('.upsell__button__content').querySelector('button[data-add-to-cart').setAttribute('data-add-to-cart-quantity', v);
          }
        }.bind(this)
      );
    ...
    listen() {
      document.addEventListener(
        ...
        function (event) {
          ...
          setTimeout(function(){
            const freeProducts = document.querySelectorAll('[data-drawer="drawer-cart"] free-product');
            const lineItems = document.querySelectorAll('[data-drawer="drawer-cart"] .cart__items .cart__items__row');
            if (freeProducts.length && lineItems.length ) {
              freeProducts.forEach((freeProduct) => {
                let isFreeSampleAdded = false;
                lineItems.forEach((lineItem) => {
                  if (lineItem.getAttribute('data-cart-item-id') == freeProduct.getAttribute('data-variant-id')) {
                    isFreeSampleAdded = true;
                  }
                });
                if (isFreeSampleAdded) {
                  freeProduct.classList.add('added');
                } else {
                  freeProduct.classList.remove('added');
                }
              });
            }
          }, 4000);
    ...
    loadForm() {
      window
        ...
        .then((response) => {
          ...
          // let sliderElement = document.querySelector('.cart__upsell__wrapper .mySwiper');
          // if (sliderElement) {
          //   setTimeout(function(){
          //     var swiper = new Swiper(".cart__upsell__wrapper .mySwiper", {
          //       slidesPerView: "auto",
          //       spaceBetween: 20,
          //       freeMode: true,
          //       pagination: {
          ...
          // sliderElement = document.querySelector('.free-sample-collection--wrapper .mySwiper');
          // if (sliderElement) {
          //   setTimeout(function(){
          //     var swiper = new Swiper(".free-sample-collection--wrapper .mySwiper", {
          //       slidesPerView: "auto",
          //       spaceBetween: 20,
          //       freeMode: true,
          //       pagination: {
  ...
  if (!customElements.get('free-product')) {
    customElements.define('free-product', FreeProduct);
  }
  
  if (!customElements.get('product-quick-add-to-cart-form')) {
    customElements.define('product-quick-add-to-cart-form', ProductQuickAddToCartForm);
  }
  ...
  document.addEventListener('change', function(e) {
    if (e.target.classList.contains('upsell-quantity-selector')) {
      e.target.closest('.upsell__button__content').querySelector('button[data-add-to-cart').setAttribute('data-add-to-cart-quantity', e.target.value);
    }
  });

  document.addEventListener('click', function(e) {
    if (e.target == document.querySelector('.gift-wrapper label[for="gift-yes"]')) {
      document.querySelector('.gift-content-wrapper').classList.remove('hide');
    } else if (e.target == document.querySelector('.gift-wrapper label[for="gift-no"]')) {
      document.querySelector('.gift-content-wrapper').classList.add('hide');
    }
  });

  var elem = document.querySelector('.ingredients--wrapper');
  ...
  // const cartDrawerCheckoutButtonTimer = setInterval(function(){        
  //   const subtotalEl = document.querySelector('.cart__drawer .subtotal .csapps-cart-total .money');
  //   const checkoutPriceEl = document.querySelector('.cart__drawer .checkout__button [data-cart-final]');

  //   if (subtotalEl && checkoutPriceEl && subtotalEl.innerHTML != checkoutPriceEl.innerHTML ) {
  //     checkoutPriceEl.innerHTML = subtotalEl.innerHTML;
  //   }    
  // }, 500);
